  ...
  return new Promise((resolve, reject) => {
    ...
    resolve(
      graphql(
        ...
        // タグ別記事ページ
        const tagsListComponent = path.resolve("./src/templates/tags-list.jsx");
        result.data.allMarkdownRemark.group.map((tagItem) => {
          createPage({
            path: `/tag/${tagItem.fieldValue}`,
            component: tagsListComponent,
            context: {
              postList: tagItem.edges,
              tagName: tagItem.fieldValue,
              siteMetadata: result.data.site.siteMetadata,
              siteTagsList: result.data.allMarkdownRemark.group,
            },
          });
          console.log(`create tag page ${tagItem.fieldValue}`);
        });
